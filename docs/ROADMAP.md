...
## Pass 3. Nonprofits, the county, the city

ICON's own record is two nonprofit projects out of three. The page has Avenue CDC and the Housing Authority and nothing else. Add as records, each on the three counts: Houston Habitat for Humanity, New Hope Housing, Houston Land Bank, Houston Community Land Trust, NRP Group (Houston office), the City's Housing and Community Development Department programmes with a unit count and a dated award, Harris County Community Services and its housing programmes, the GLO disaster-rebuild programmes with a Harris County unit count. Sources: the organisation's own annual report or programme page, the city or county agenda item (the PDF on the agency site), the GLO programme page. For a public body the decider is the procurement route and the named programme director, not an executive. Note which programmes carry Buy America or federal procurement rules that touch a wall system; that is a code question and belongs in `code.py`, with the regulation named.

## Pass 4. Build-to-rent operators
...
## Pass 6. Open items from the handoff
...
## Pass 7. The payback page (inputs only)
